...
| **ISS-16** | UI | `UI-DA01` | Tra cứu | Bảng văn bản | Xem & Thao tác Header bảng | Desktop | Major | Header bảng dữ liệu | Header gộp 2 trường (`Ngày đến ⇅ - Số đến đi ⇅`) nhồi nhét 2 nút sort sát nhau vi phạm target size, dễ bấm nhầm và gây rối mắt | Tách thành các cột độc lập hoặc chỉ sort trường chính; chỉ hiện icon sort khi hover/active | Loại bỏ hoàn toàn nguy cơ click nhầm, giao diện thanh thoát | Major | FAIL | 2 | 2 | 3.0 (P2) |
...
| **ISS-16** | UI | `UI-DA01` | Bảng văn bản | Đọc & Sort header bảng | Desktop | Major | Header bảng dữ liệu | 2 nút sort `⇅` đặt sát nhau trên cùng ô dễ gây bấm nhầm | Thiết kế lại phân tách cột hoặc chỉ sort trường chính, ẩn icon khi tĩnh | Dễ quét dữ liệu theo cột, loại bỏ nguy cơ click nhầm | **P2** |
